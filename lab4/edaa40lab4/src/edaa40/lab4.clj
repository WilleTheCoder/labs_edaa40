...
(defn create-square-sum-relation
    ...
    [A]
    (set( for[x A y A :when (square? (+ x y))]
    [x y]))
...
(defn- H'
    ...
    ;;  a is the current node. (start node pos)
    ...
    (let[x (intersection S (image-of E a))] ;;unvisited node neighbour

    (some #(H' E % (disj S %) (concat P[a])) x)
    )
    )
