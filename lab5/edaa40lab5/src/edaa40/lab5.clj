...
(declare insert-into-queue);;this
...
(defn insert-into-queue 
    ...
    (sort-by :frequency (concat Q (list T)))
)
...
(defn huffman-encode
    ...
    (let [
      tree (huffman-tree S) ;; the tree
      length (count S)  ;;the length of the sequence, number of elements in the seq
      codeseq (huffman-codes tree) ;{:a (100)} i.e
      bits (mapcat codeseq S) ;;sequence defined in bits S->symbol | mapcat adds the sequences to one list
    ]
...
(declare decode-symbol);;this hmm
