...
(defn cartesian
  ...
  ;; combining every element in A with every element in B. 
...
(defn rng
  ...
  ;; Set of all righthand values in the relation, (a,b)
  [R]
...
(defn image-of 
  ...
  ;; The set of righthand values (range) in a relation that corresponds to the domain x.
  ;; R(a) = {b: aRb}
  [R x] 
...
(defn inverse 
  ...
  ;; domains change position with ranges in a relation aRb.
  [R]

  (set(for [x R] [(second x) (first x)] ))
...
(defn reflexive?
  ...
  ;; A relation R on a set A is reflexive if it relates every element of A to itself.
...
(defn symmetric?
  ...
  ;; if aRb -> bRa
  ;; arrow back
  ;; a -> a doesnt break symmetry
  ;; inv1 inverts array [2 1] -> [1 2]
  [R]
...
(defn transitive?
  ...
  ;; if aRb and bRc then must aRc
  [R]
 (every? #(subset?
                (image-of R (second %))
                (image-of R (first %))) R) 
                
                ;; check if R(b) is subset of R(a) for all (a,b) in R
...
(defn surjective? 
  ...
  ;; every element in codomain B is being mapped to in function f.
  ;;rng of f = #{1 2 3}
